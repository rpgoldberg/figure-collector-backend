#!/bin/bash
# WSL Path Fix Script for Jest Testing
...
# Try to install with workaround for mongodb-memory-server UNC path issues
echo "Installing with mongodb-memory-server workaround..."
npm install --ignore-scripts

# Then run postinstall scripts manually for packages that work
echo "Running necessary postinstall scripts..."
